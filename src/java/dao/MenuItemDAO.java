...
public class MenuItemDAO {
    ...
    String query = "SELECT ItemID, CategoryID, ItemName, Description, Price, IsAvailable, ImageURL " +
...
// Update MenuItemDAO.java - getMenuItemById method
public MenuItem getMenuItemById(int itemId) throws SQLException {
    ...
    List<MenuItem> items = new ArrayList<>();
    String query = "SELECT ItemID, CategoryID, ItemName, Description, Price, IsAvailable, ImageURL " +
                  "FROM MenuItems ORDER BY CategoryID, ItemName";
...
// Add a new menu item
public int addMenuItem(int categoryId, String itemName, String description, double price, boolean isAvailable, String imageUrl) throws SQLException {
    String query = "INSERT INTO MenuItems (CategoryID, ItemName, Description, Price, IsAvailable, ImageURL) " +
                  "VALUES (?, ?, ?, ?, ?, ?); SELECT SCOPE_IDENTITY() AS ItemID";
...
public void updateMenuItem(int itemId, int categoryId, String itemName, String description, double price, boolean isAvailable, String imageUrl) throws SQLException {
    String query = "UPDATE MenuItems SET CategoryID = ?, ItemName = ?, Description = ?, " +
                  "Price = ?, IsAvailable = ?, ImageURL = ? WHERE ItemID = ?";
...
public void deleteMenuItem(int itemId) throws SQLException {
    // First check if this item is used in any order
    String checkQuery = "SELECT COUNT(*) FROM OrderItems WHERE ItemID = ?";
    ...
    try (Connection conn = DatabaseConnection.getConnection();
        ...
        try (ResultSet rs = checkStmt.executeQuery()) {
            if (rs.next() && rs.getInt(1) > 0) {
                // Item is in use, just mark as unavailable instead of deleting
                String updateQuery = "UPDATE MenuItems SET IsAvailable = 0 WHERE ItemID = ?";
                try (PreparedStatement updateStmt = conn.prepareStatement(updateQuery)) {
                    updateStmt.setInt(1, itemId);
                    updateStmt.executeUpdate();
                }
            } else {
                // Item is not in use, safe to delete
                String deleteQuery = "DELETE FROM MenuItems WHERE ItemID = ?";
